...
public class OperationClass {
    ...
    private static void formatPolynomialTerm(int sign, int degree, double coefficient, StringBuilder finalText) {
    ...
    private static String displayFinalPolynomial(HashMap<Integer, Double> finalMap) {
        ...
        for (HashMap.Entry<Integer, Double> entry : finalMap.entrySet()) {
            ...
            formatPolynomialTerm(sign, degree, coefficient, finalText);
        }
    ...
    private static Polynomial additionOfPolynomials(Polynomial poly1, Polynomial poly2) {
    ...
    private static Polynomial subtractionOfPolynomials(Polynomial poly1, Polynomial poly2) {
        HashMap<Integer, Double> firstMap = poly1.getCoefficientMap();
        HashMap<Integer, Double> secondMap = poly2.getCoefficientMap();
        HashMap<Integer, Double> resultingMap = new HashMap<>();
        for (HashMap.Entry<Integer, Double> entry : firstMap.entrySet()) {
    ...
    private static Polynomial multiplicationOfPolynomials(Polynomial poly1, Polynomial poly2) {
        HashMap<Integer, Double> firstMap = poly1.getCoefficientMap();
        HashMap<Integer, Double> secondMap = poly2.getCoefficientMap();
        HashMap<Integer, Double> resultingMap = new HashMap<>();
    ...
    private static Polynomial[] divisionOfPolynomials(Polynomial poly1, Polynomial poly2) {
        ...
        while (highestPowerRemainder >= highestPowerSecond) {
            ...
            double coefficient;
            coefficient = (secondMap.getOrDefault(highestPowerSecond, (double) 0) != 0) ? remainderMap.get(highestPowerRemainder) / secondMap.get(highestPowerSecond) : 0;
            ...
            for (HashMap.Entry<Integer, Double> entry : secondMap.entrySet()) {
                ...
                remainderMap.put(newDegree, remainderMap.containsKey(newDegree) ? remainderMap.get(newDegree) - newCoefficient : -newCoefficient);
            }
        ...
        if (remainderMap.isEmpty() || remainderMapString.equals("0"))
            return new Polynomial[]{createFinalPolynomialOfTypePolynomial(displayFinalPolynomial(resultingMap), resultingMap), null, null};
    ...
    private static Polynomial differentiationOfPolynomial(Polynomial poly1) {
        HashMap<Integer, Double> firstMap = poly1.getCoefficientMap();
        HashMap<Integer, Double> resultingMap = new HashMap<>();
        for (HashMap.Entry<Integer, Double> entry : firstMap.entrySet()) {
            if (entry.getKey() != 0) {
    ...
    private static Polynomial integrationOfPolynomial(Polynomial poly1) {
        HashMap<Integer, Double> firstMap = poly1.getCoefficientMap();
        HashMap<Integer, Double> resultingMap = new HashMap<>();
        for (HashMap.Entry<Integer, Double> entry : firstMap.entrySet()) {
            if (entry.getValue() != 0)
